...
public class MyTest3 {
     ...
     *
     * =====================================
     * 在JDK 1.5之前,我们若想实现线程同步,只能通过synchronized关键字这一种方式来达成;底层Java也是通过synchronized关键字来做到数据的原子性维护的;
     * synchronized关键字是JVM实现的一种内置锁,从底层角度来说,这种锁的获取与释放都是由JVM帮助我们隐式实现的。
     *
     * 从JDK 1.5开始,并发包引入了Lock锁,Lock同步锁是基于Java来实现的,因此锁的获取与释放都是通过Java代码来实现与控制的;
     * 然而synchronized是基于底层操作系统的Mutex Lock来实现的,每次对锁的获取与释放动作都会带来用户态与内核态之间的切换,
     ...
     * 因此它是偏向锁,而第2个线程在争抢时,会发现该对象头中的Mark Word已经是偏向锁,但里面存储的线程ID并不是自己(是第1个线程),
