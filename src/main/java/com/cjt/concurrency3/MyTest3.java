...
public class MyTest3 {
     ...
     *
     * 那些处于EntryList与waitSet中的线程均处于阻塞状态,阻塞操作是由操作系统来完成的,在linux下是 通过pthread mutex_lock函数实现的。
     ...
     *
     * 互斥锁的属性:
     * 1. PTHREAD MUTEX TIMED NP:这是缺省值,也就是普通锁。当一个线程加锁以后,其余请求锁的线程将会形成一个等待队列,并且在解锁后按照优先级获取到锁。
     *    这种策略可以确保资源分配的公平性
     * 2. PTHREAD MUTEX RECURSIVE_NP: 嵌套锁。允许一个线程对同一个锁成功获取多次,并通过unlock解锁。如果是不同线程请求,则在加锁线程解锁时重新进行竞争。
     * 3. PTHREAD MUTEX. ERRORCHECK NP: 检错锁。如果一个线程请求同一个锁,则返回EDEADLR, 否则与PTHREAD_MUTEX_TIMED_NP类型动作相同,
     *    这样就保证了当不允许多次加锁时不会出现最简单情况下的死锁。
     * 4. PTHREAD_MUTEX ADAPTIVE NP: 适应锁,动作最简单的锁类型,仅仅等待解锁后重新竞争。
     *
     * =====================================
